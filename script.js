...
// ==================== API CONFIGURATION ====================
/**
 * API Keys must be defined in config.js
 * config.js is:
 * - Generated by build.js from environment variables (Vercel deployment)
 * - Manually created from config.example.js (local development)
 * - Always in .gitignore (never committed)
 */

// Verify API keys are loaded
if (typeof API_KEYS === 'undefined') {
    console.error('❌ API_KEYS not found! Make sure config.js is loaded.');
    throw new Error('API Configuration missing. See console for details.');
}

if (!API_KEYS.OPENROUTER_KEY || API_KEYS.OPENROUTER_KEY === 'your-openrouter-key-here') {
    throw new Error('❌ OPENROUTER_KEY not set. Check config.js');
}

if (!API_KEYS.SERPAPI_KEY || API_KEYS.SERPAPI_KEY === 'your-serpapi-key-here') {
    throw new Error('❌ SERPAPI_KEY not set. Check config.js');
}
